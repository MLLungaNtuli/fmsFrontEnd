      ...
      <div class="main-content" :class="{ 'sidebar-collapsed': !isSidebarOpen }">
        <div class="content-container">
          <div class="header">
            <h1>Edit Driver</h1>
            <div class="actions">
              <Button label="Save" icon="pi pi-check" @click="saveDriver" class="p-button-primary" :loading="saving" />
              <Button label="Cancel" icon="pi pi-times" @click="cancelEdit" class="p-button-secondary" />
            </div>
          </div>

          <div class="form-container">
            <DriverForm v-model:driver="driver" :errors="errors" />
          </div>
        </div>
      </div>
    </div>
...
const saveDriver = async () => {
  saving.value = true;
  try {
    await api.put(`/drivers/${id}`, driver.value);
